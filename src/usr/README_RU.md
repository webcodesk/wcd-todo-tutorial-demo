...
Переименуйте новый экземпляр на `newNotePageRouteAnchor` и установите значение `/new-note` в свойство `Page Route Path`.

(имаж)  

Теперь вы можете перейти на закладку `go-to-new-note-page` flow диаграммы. 
Перетащите `newNotePageRouteAnchor` экземпляр куда-то на свободное место на диаграмме. 
Соедините выход `pageRouteAbchorProps` функции `goToPage` со входом `props` экземпляра `newNotePageRouteAnchor`.

Благодаря этому соединению функция может прочитать значение свойства `Page Route Path` и перейти по указанному там адресу.
...
Но перед этим, вам необходимо добавить экземпляр компонента `PageRouterAnchor` на страницу `new-note` для перехода на `main` страницу.

Откройте закладку страницы `new-note` и добавьте item в свойство `Hidden Components` в экземпляре `newNotePageFrame`.
Перетащите компонент `PageRouterAnchor` в новый item, и переименуйте экземпляр на `mainPageRouteAnchor`.
Установите значение "/" в свойство `Page Route Path`.

(имаж)

Создайте новую flow диаграмму с именем `cancelling-new-note`. 
Земените элемент `Application` на экземпляр `newNoteForm`.
Добавьте функцию `goToPage` в диаграмму и соедините ее вход с выходом `onCancel` экземпляра `newNoteForm`.

> Событие `onCancel` происходит когда пользователь кликает на кнопку `Cancel` в форме `newNoteForm`

Добавьте экземпляр `mainPageRouteAnchor` (из секции "Pages" и страницы `new-note`) в диаграмму. 
Соедините выход `pageRouteAnchorProps` функции `goToPage` с входом `props` экзепляра `mainPageRouteAnchor`.
...
Подсоедините выход `onApplicationStart` элемента `Application` ко входу функции `getNotes`. 
...
Соедините выход `notes` функции `getNotes` со входом функции `putIntoNotesBuffer`.

(имаж)

Буфер памяти вам необходим для минимизации количества вызовов функций для обновления компонента `mainPageNotesList`.
...
Иначе вам бы пришлось при каждой операции со списком добавлять вызовы функций для обновления списка в экземпляре `mainPageNotesList`.

Создайте новую flow диаграмму с именем `update-notes-by-navigation-filter`. 
Это имя выбрано из-за того, что список должен быть всегда отфильтрован по выбраному типу фильтра в экземпляре `mainPageNavigationTabs`.

Добавьте функцию `listenToNotesBuffer` в диаграмму и соедините ее вход с выходом `onApplicationStart` элемента `Application`.
Таким образом, вы указали, что при запуске приложения в браузере необходимо начать следить за изменениями в буфере.

(имаж)

Теперь вам необходимо сказать что делать при изменении списка в буфере.
Добавьте функцию `filterNotes` и соедините ее вход с выходом `updatedData` функции `listenToNotesBuffer`.
Добавьте `mainPageNotesList` экземпляр и соедините его вход с выходом `todoNotesListProps` функции `filterNotes`.
Также добавьте `mainPageNavigationTabs` экземпляр и соедините его вход с выходом `navigationTabsProps` функции `filterNotes`.

(имаж)

Вы наверное догадались, что в данном случае `filterNotes` 
функция прочитает все свойства `mainPageNavigationTabs` экземпляра и в зависимости от значения 
в `Active Tab Type` свойстве отфильтрует обновленный список из буфера и передаст его в свойства `todoNotesListProps` экземпляра.   

Вам нужно еще закончить flow где список в буфере фильтруется в зависимости от выбранного фильтра.
Поэтому откройте `change-navigation-filter-by-click` flow диаграмму и добавьте в нее функцию `getNotes`.
Выход `onActiveTabUpdated` экземпляра `mainPageNavigationTabs` (последней в цепочке) соедините с входом функции `getNotes`.
...
Замените `Application` на экземпляр `newNoteForm`. 
Добавьте `validateNoteText` функцию в диаграмму. 
Соедините выход `onSaveNote` экземпляра `newNoteForm` с входом функции `validateNoteText`.
...
А выход `newNoteFormProps` функции `setError` подключить ко входу `props` экземпляра `newNoteForm`.
...
Осталось сделать еще одну flow диаграмму с удалением записи и изменением статуса записи в списке.
...
Замените элемент `Application` экземпляром `mainPageNotesList`.
Добавьте функцию `deleteNote` и соедините ее вход с выходом `onDeleteNote` экземпляра `mainPageNotesList`.
Так же добавьте функцию `saveNotes` и соедините ее вход в выходом `notes` функции `deleteNote`.
И как вы уже догадались наверное, необходимо обновить записи в буфере. 
Поэтому перетяните функцию `putIntoNotesBuffer` и соедините ее вход с выходом `notes` функции `saveNotes`.
Ну что же, вы сделали удаление записей по клику на кнопку удаления в списке на главной странице.

(имаж)

Теперь опишем часть flow для изменения статуса записи в списке по клику на checkbox.
Добавьте функцию `toggleNodeCompleted` и соедините ее вход с выходом `onToggleNoteCompleted` экземпляра `mainPageNotesList`.
Затем добавьте еще один элемент функции `saveNotes` и соедините ее вход с выходом `notes` функции `toggleNodeCompleted`.

И конечно же, необходимо передать новый список в буффер.
Перетяните функцию `putIntoNotesBuffer` и соедините ее вход с выходом `notes` функции `saveNotes`.
